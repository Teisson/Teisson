# Data Analytics Engineering Portfolio – Teis Nilsson

Welcome!  
This portfolio highlights my practical experience in **data engineering, marketing analytics, tracking architecture, and automation** — with a strong foundation in **Python, SQL, and first-party data systems**.

Drawing from my background in IT, cybersecurity, and 6+ years of running a marketing agency, and several years as a Salesforce developer I specialize in building **secure, reliable and activation-ready data pipelines** that turn raw marketing data into measurable business impact.

---

## Projects Included

### 1. Marketing Data Pipeline – GA4 → BigQuery → dbt Models
A production-style pipeline that ingests GA4 event data into BigQuery, transforms it using dbt, and produces clean, analytics-ready tables for attribution, funnel analysis and audience segmentation.

**Tech used:**  
BigQuery • dbt • SQL • Python • GA4 export • Git • CI/CD

**Key features:**  
- Unified session + event model  
- Marketing channel mapping  
- Data quality tests (dbt tests)  
- Incremental models for scalable performance  
- Funnel & attribution-ready marts

*See folder:* `ga4_bq_dbt_pipeline/`

---

### 2. First-Party CRM Segmentation & Predictive Scoring
A Python + SQL project that prepares CRM exports (Salesforce-style) for segmentation, cleaning, identity resolution, and basic predictive scoring (likelihood to convert).

**Tech used:**  
Python (pandas) • SQL • BigQuery • Scikit-learn (simple baseline model)

**Key features:**  
- Data cleaning + deduplication  
- Normalization & tokenization for privacy-safe activation  
- Audience scoring based on behavioral + transactional features  
- Ready for export to ad platforms or marketing automation

*See folder:* `crm_segmentation_scoring/`

---

## Tech & Tools
**Languages:** Python, SQL  
**Cloud & Warehouses:** BigQuery, GCP  
**Data Modelling:** dbt (Fundamentals), marketing data schemas  
**Tracking:** GA4, GTM, server-side tagging  
**Marketing Systems:** CRM platforms, e-commerce tracking, audience segmentation  
**Version Control:** Git/GitHub  
**Security Mindset:** GDPR, IAM basics, data governance

---

## Contact
If you're exploring marketing data engineering talent with a strong mix of data, marketing, and security skills — feel free to reach out.

**LinkedIn:** <https://www.linkedin.com/in/teis-nilsson/>  
**Email:** <teisnilsson@gmail.com>  

